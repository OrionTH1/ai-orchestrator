...
A ordem intuitiva seria construir o pipeline na ordem em que ele roda: PRD, depois Techspec, depois decomposição, depois execução, e por fim a validação.
...
O **Validar** é a parte nova, é o diferencial, e é onde mora o risco técnico. Por isso ele vem primeiro, alimentado por um PRD escrito à mão. Se ele não funcionar — ou não for gostoso de usar — o projeto morre na semana 3, não no mês 4.
...
## Fase 2 — VALIDAR (a fatia que prova a tese)
...
- **Validar comportamento**: subir dev server na porta daquela Task, esperar responder, abrir o navegador.
...
- **Step PRD**: usuário digita a ideia → agente entrevista sobre ideia e regras de negócio → produz **um** documento com:
  ...
  - **Critérios de aceite** — o artefato que alimenta o step Validar.
...
- **Memória de projeto (camada 1: fatos).** Poucos fatos curtos e estáveis — escala do produto, stack, convenções — gravados no repo e lidos por toda Task nova. É o que impede o agente de perguntar a escala do projeto pela décima vez. Editável à mão. Entra inteiro no contexto: é pequeno de propósito.
